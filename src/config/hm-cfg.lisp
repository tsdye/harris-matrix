...
;; 6 = graph-class-attribute
;; 7 = domain
;; 8 = default-value
;; use columns 2 and 3 to lookup .ini file section and option in columns 0 and 1
(defun master-table ()
'(("Output files" "sequence-dot" "general" "none" "sequence" "none" "none" "output" "" )
("Output files" "chronology-dot" "general" "none" "chronology" "none" "none" "output" "" )
("Input files" "contexts" "general" "none" "sequence" "none" "none" "input" "" )
("Input files" "observations" "general" "none" "sequence" "none" "none" "input" "" )
("Input files" "inferences" "general" "none" "sequence" "none" "none" "input" "" )
("Input files" "periods" "general" "none" "sequence" "none" "none" "input" "" )
("Input files" "phases" "general" "none" "sequence" "none" "none" "input" "" )
("Input files" "events" "general" "none" "chronology" "none" "none" "input" "" )
("Input files" "event-order" "general" "none" "chronology" "none" "none" "input" "" )
("Input file headers" "contexts" "general" "none" "sequence" "none" "none" "header" "" )
("Input file headers" "observations" "general" "none" "sequence" "none" "none" "header" "" )
("Input file headers" "inferences" "general" "none" "sequence" "none" "none" "header" "" )
("Input file headers" "periods" "general" "none" "sequence" "none" "none" "header" "" )
("Input file headers" "phases" "general" "none" "sequence" "none" "none" "header" "" )
("Input file headers" "events" "general" "none" "chronology" "none" "none" "header" "" )
("Input file headers" "event-order" "general" "none" "chronology" "none" "none" "header" "" )
("General configuration" "chronology-graph-draw" "general" "none" "chronology" "none" "none" "general" "off" )
("General configuration" "url-include" "general" "none" "sequence" "none" "none" "general" "off" )
("General configuration" "url-default" "general" "none" "sequence" "none" "none" "general" "http://tsdye.github.io/harris-matrix/" )
("General configuration" "legend" "general" "none" "all" "none" "none" "general" "off" )
("General configuration" "assume-correlations" "general" "none" "sequence" "none" "none" "general" "no" )
("General configuration" "fast-matrix" "general" "none" "sequence" "none" "none" "general" "on" )
("General configuration" "add-interfaces" "general" "none" "sequence" "none" "none" "general" "no" )
...
(defun penwidth-min-max (cfg element)
  "Returns a list with penwidth-min and penwidth-max for the ELEMENT from the
  user's configuration, CFG. ELEMENT is one of `edge', `node'."
  (let ((section (if (string= element "edge")
                     "Graphviz sequence edge attributes"
                     "Graphviz sequence node attributes")))
    (list (get-option cfg section "penwidth-min")
...
(defun make-lookup-map (master)
  "Return an fset map whose key is the set (element dot-attr graph-type) and
whose value is a two element list with the configuration section and option."
  (let ((map (fset:empty-map)))
    (dolist (row master)
      (let ((key (fset:empty-set)))
        (dolist (col '(2 3 4))
...
(defun section-name (element dot-attr graph-type)
  (let ((map (make-lookup-map (master-table))))
    (nth 0 (fset:lookup map (fset:set element dot-attr graph-type)))))

(defun option-name (element dot-attr graph-type)
  (let ((map (make-lookup-map (master-table))))
    (nth 1 (fset:lookup map (fset:set element dot-attr graph-type)))))

(defun lookup-graphviz-option (cfg element dot-attr graph-type)
  ...
  (get-option cfg (section-name element dot-attr graph-type)
              (option-name element dot-attr graph-type)))
