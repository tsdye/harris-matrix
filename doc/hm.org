...
* Harris matrix
** DONE Modify lookup table
   CLOSED: [2017-11-25 Sat 09:42] SCHEDULED: <2017-11-24 Fri>
Add node and edge fontsize min and fontsize max

Take out the various node and edge penwidths, the implementation only uses the
min and max penwidths from the general attributes.
** Master table
*** Lookup table
#+name: hm-lookup-table
| user-section                                            | user-option                 | element | dot-attribute           | graph-type | graph-classification        | graph-class-attribute | default-value                         |
|---------------------------------------------------------+-----------------------------+---------+-------------------------+------------+-----------------------------+-----------------------+---------------------------------------|
| Output files                                            | sequence-dot                | none    | none                    | sequence   | none                        | none                  |                                       |
...
*** Code to write out lookup table

#+name: write-hm-lookup-table-to-lisp
#+header: :colnames yes
#+header: :var x=hm-lookup-table
#+header: :var varname="master-table"
#+header: :wrap SRC lisp
#+BEGIN_SRC emacs-lisp
  (let ((res (format "(defun %s ()\n(list " varname)))
    (dolist (y x)
      (setf res (concat res "'("))
      (dolist (z y)
        (setf res (concat res (cond
                               ((or (eq (first y) z) (eq (second y) z))
                                (format "\"%s\" " z))
                               ((eq (car (last y)) z)
                                (format "\"%s\"" z))
                               (t (format ":%s " z))))))
      (setf res (if (eq (car (last x)) y)
...
#+RESULTS: write-hm-lookup-table-to-lisp
#+BEGIN_SRC lisp
(defun master-table ()
...
*** RGB v. 1.2
#+name: v-1.2
#+header: :var x=x11 :wrap SRC lisp
#+BEGIN_SRC lisp
  (let ((ret "(defun x11-colors ()
(fset:set "))
    (dolist (color x)
      (unless (position #\Space (car color))
        (setf ret (concatenate 'string
                   ret (format nil "~s " (string-downcase (car color)))))))
...
******* Edge
****** JSON
***** DONE write-default-ini
      CLOSED: [2017-01-22 Sun 06:44]
      :LOGBOOK:
      - State "DONE"       from "TODO"       [2017-01-22 Sun 06:44]
      :END:
 This function should write the default INI file used by the program.
**** PyConfigParser
***** Get rid of global variables, read from config instead

** HM github manual
   :PROPERTIES:
   :EXPORT_FILE_NAME: index
   :EXPORT_TITLE: HM: Directed Graph Representation of Archaeology's Harris Matrix
   :END:
...
[[file:~/quicklisp/local-projects/hm/examples/fig-12-chronology/fig-12-section-w-interfaces.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/fig-12-chronology/fig-12-sequence.png][file:~/quicklisp/local-projects/hm/examples/fig-12-chronology/fig-12-sequence-small.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/fig-12-chronology/fig-12-correlations.png][file:~/quicklisp/local-projects/hm/examples/fig-12-chronology/fig-12-correlations-small.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/roskams-h/fig-14.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/roskams-h/fig-14-h-structure.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/roskams-h/roskams-h-sequence.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/roskams-jumps/roskams-jumps-illustration.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/roskams-jumps/roskams-jumps-sequence.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/h-structure/complex-h-structure.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/h-structure/complex-h-structure-reachable-2.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/bldg-1-5/bldg-1-5-sequence.png][file:~/quicklisp/local-projects/hm/examples/bldg-1-5/bldg-1-5-sequence-small.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/bldg-1-5/bldg-1-5-chronology.png][file:~/quicklisp/local-projects/hm/examples/bldg-1-5/bldg-1-5-chronology-small.png]]
*** Classifiers
...
[[file:~/quicklisp/local-projects/hm/examples/fig-12-chronology/fig-12-periods.png][file:~/quicklisp/local-projects/hm/examples/fig-12-chronology/fig-12-periods-small.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/roskams-h/roskams-h-sequence-solarized-dark.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/bldg-1-5/bldg-1-5-sequence-distance-1332+.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/bldg-1-5/bldg-1-5-sequence-levels.png][file:~/quicklisp/local-projects/hm/examples/bldg-1-5/bldg-1-5-sequence-levels-small.png]]
...
#+BEGIN_SRC sh
head ~/quicklisp/local-projects/hm/examples/fig-12-chronology/fig-12-periods.csv
#+END_SRC
...
****** Solarized
 ...
 [[file:~/quicklisp/local-projects/hm/examples/roskams-h/roskams-h-sequence-solarized-light.png]]
...
[[file:~/quicklisp/local-projects/hm/examples/fig-12-chronology/fig-12-polygon-1.png][file:~/quicklisp/local-projects/hm/examples/fig-12-chronology/fig-12-polygon-1-small.png]]
