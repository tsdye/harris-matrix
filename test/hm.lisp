...
(defixture transitive-graph
  (:setup (setf *digraph* (graph:populate (make-instance 'graph:digraph)
                                          :nodes '(a b c)
                                          :edges '((a b)
                                                   (b c)
                                                   (a c)))
                *cfg* (make-default-configuration)))
  (:teardown (setf *digraph* nil
                   *cfg* nil)))

(defixture default-configuration
  (:setup (setf *cfg* (make-default-configuration)))
  (:teardown (setf *cfg* nil)))

;;; Tests for internal functions

(deftest fast-matrix-test ()
  (with-fixture default-configuration
...
(deftest correlated-node-test ()
  (with-fixture transitive-graph
    (is (eq (hm::correlated-node (first (graph:nodes *digraph*))
                                 (second (graph:nodes *digraph*))
                                 nil)
            'a=b))
    (is (string= (hm::correlated-node (first (graph:nodes *digraph*))
                                 (second (graph:nodes *digraph*))
                                 t)
            "A=B"))))
...
(deftest node-index-test ()
  (with-fixture transitive-graph
    (let ((i (hm::make-node-index *digraph*)))
      (is (eq (first (graph:nodes *digraph*))
              (fset:lookup i 0)))
      (is (eq (second (graph:nodes *digraph*))
              (fset:lookup i 1)))
      (is (eq (third (graph:nodes *digraph*))
              (fset:lookup i 2))))))

(deftest transitive-reduction-new ()
  (with-fixture transitive-graph
    (is (tree-equal (graph:edges (hm::transitive-reduction-new
                           *digraph*
                           *cfg*))
                     '((a b) (b c))))))
...
(deftest transitive-reduction ()
  (with-fixture transitive-graph
    (is (tree-equal (graph:edges (hm::transitive-reduction
                                  *digraph*
                                  *cfg*
                                  nil))
                    '((a b) (b c))))))
...
(deftest test-reset-option ()
  (let ((default-config (make-default-configuration)))
    (with-fixture default-configuration
      (is (equal (get-all-configuration-options *cfg*)
