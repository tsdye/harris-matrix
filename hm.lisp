...
(defmacro <-dot-edge (seq classifier gv-attr gv-default)
  `(let* ((cfg (archaeological-sequence-configuration ,seq))
          ...
          (class-with-node
            (get-option cfg "Graphviz sequence classification" "edge-classify-by"))
          (value (when ,classifier
                   (get-option cfg "Graphviz sequence classification" ,classifier))))
     (if (or (not ,classifier) (and ,classifier (emptyp value))
             (not (fset:domain-contains? classifiers value)))
         (let ((y (process-user-value
                   (lookup-option map attribute) attribute sequence)))
           (constantly (if (emptyp y) def y)))
         (cond
           ((string= "units" value)
            (lambda (x)
              (let ((user-value
                      (if (= 0 (fset:@ (fset:@ classifiers ,classifier)
                                        (if (string= "to" class-with-node)
                                            (nth 1 (edge-value graph x))
                                            (nth 0 (edge-value graph x)))))
                          (lookup-option map value ,classifier "deposit")
                          (lookup-option map value ,classifier "interface"))))
                (if (emptyp user-value)
                    (error
                     "Unable to set edge function.  User value not set for ~s.~&"
                     ,classifier)
                    (process-user-value user-value ,classifier ,seq :element :edge)))))
           ((string= "adjacent" value)
            (lambda (x)
              (let ((user-value
                      (case (fset:@ (fset:@ classifiers ,classifier)
                                     (if (string= "to" class-with-node)
                                         (nth 1 (edge-value graph x))
                                         (nth 0 (edge-value graph x))))
                        (0 (lookup-option map value ,classifier "origin"))
                        (1 (lookup-option map value ,classifier "adjacent"))
                        (t (lookup-option map value ,classifier "not-adjacent")))))
                (if (emptyp user-value)
                    (error
                     "Unable to set edge function.  User value not set for ~s.~&"
                     ,classifier)
                    (process-user-value user-value ,gv-attr ,seq :element :edge)))))
            ...
            (lambda (x)
              (let ((user-value
                      (case (fset:@ (fset:@ classifiers ,classifier)
                        ...
                        (0 (lookup-option map value ,classifier "origin"))
                        (1 (lookup-option map value ,classifier "adjacent"))
                        (2 (lookup-option map value ,classifier "reachable"))
                ...
                (if (emptyp user-value)
                    (error
                     ...
                     ,classifier)
                    (process-user-value
                     user-value ,gv-attr ,seq :element :edge)))))
           ((fset:contains?
             (fset:set "distance" "levels" "periods" "phases") value)
            (lambda (x) (edge--process-matrix-value
                         (fset:@ (fset:@ classifiers value)
                                  (if (string= "to" class-with-node)
                                      (nth 1 (edge-value graph x))
                                      (nth 0 (edge-value graph x))))
                         ,gv-attr ,classifier ,seq)))
           (t (error "Error: Unable to set edge function.~&"))))))

(defmacro <-dot-node (seq classifier gv-attr gv-default)
  `(let* ((sequence ,seq)
          (opt ,classifier)
          (attribute ,gv-attr)
          (def ,gv-default)
     ...
     (if (or (not opt) (and opt (emptyp value))
             (not (fset:domain-contains? classifiers value)))
         (let ((y (process-user-value
                   (lookup-option map attribute) attribute sequence)))
           (constantly (if (emptyp y) def y)))
         (cond
           ((string= "units" value)
            (lambda (x)
              (let ((user-value
                      (if (= 0 (fset:@ (fset:@ classifiers value) x))
                          (lookup-option map value opt "deposit")
                          (lookup-option map value opt "interface"))))
                (if (emptyp user-value)
                    (error
                     "Unable to set node function.  User value not set for ~s.~&"
                     opt)
                    (process-user-value
                     user-value attribute sequence :element :node)))))
           ((string= "adjacent" value)
            (lambda (x)
              (let ((user-value
                      (case (fset:@ (fset:@ classifiers value) x)
                        (0 (lookup-option map value opt "origin"))
                        (1 (lookup-option map value opt "adjacent"))
            ...
            (lambda (x)
              (let ((user-value
                      (case (fset:@ (fset:@ classifiers value) x)
                        (0 (lookup-option map value opt "origin"))
                        (1 (lookup-option map value opt "adjacent"))
                        (2 (lookup-option map value opt "reachable"))
           ...
           ((fset:contains?
             (fset:set "distance" "levels" "periods" "phases") value)
            (lambda (x)
              (node--process-matrix-value
               (fset:@ (fset:@ classifiers value) x) attribute value sequence)))
           (t (error "Error: Unable to set node function.~&"))))))

(defun process-user-value (value gv-attr seq &key element)
  (if (fset:contains? (fset:set "color" "fillcolor" "fontcolor") gv-attr)
      (let* ((section (if (equal element :node)
             ...
             (scheme (get-option
                      ...
                      section "colorscheme")))
        (if (and (not (emptyp value)) (not (emptyp scheme)))
            (quotes-around (graphviz-color-string value scheme)) ""))
      (quotes-around value)))
...
Graphviz dot value."
  (let ((range
          (fset:range
           (fset:@ (archaeological-sequence-classifiers seq) option))))
    (cond
      ((string= attr "style")
       (quotes-around (graphviz-edge-style value)))
      ((fset:contains? (fset:set "color" "fontcolor") attr)
       (quotes-around
        (graphviz-color-string
         value
         (get-option
          (archaeological-sequence-configuration seq)
          "Graphviz sequence edge attributes" "colorscheme")
         (1+ (- (fset:greatest range) (fset:least range))))))
      ((string= attr "penwidth")
       (let ((interval
               (- (get-option (archaeological-sequence-configuration seq)
                              "Graphviz sequence edge attributes"
                              "penwidth-max" :type :number)
                  (get-option (archaeological-sequence-configuration seq)
                              "Graphviz sequence edge attributes"
                              "penwidth-min" :type :number)))
...
Graphviz dot value."
  (let ((range
    ...
    (cond
      ...
      ((fset:contains? (fset:set "color" "fillcolor" "fontcolor") attr)
       (let ((scheme (get-option
                      (archaeological-sequence-configuration seq)
                      "Graphviz sequence node attributes" "colorscheme")))
         (quotes-around (graphviz-color-string value scheme
                                               (1+ (- (fset:greatest range)
                                                      (fset:least range)))))))
      ((string= attr "penwidth")
       (let ((interval
         ...
         (quotes-around (+ base
                           (* value
                              (/ interval (1+ (- (fset:greatest range)
                                                 (fset:least range))))))))))))

(defun <-dot-graph (user default)
  (if (emptyp user) (quotes-around default)
      (quotes-around user)))
...
(defun graphviz-edge-style (index)
  ...
  (let ((map
          (-> (fset:empty-map 0)        (fset:with 0 "solid")
              (fset:with 1 "dashed")    (fset:with 2 "dotted")
              (fset:with 3 "bold"))))
...
(defun make-edge-lookup-map (cfg)
  (-> (fset:empty-map 0)
      (fset:with
       (concatenate 'string "units" "edge-color-by" "deposit")
       (get-option cfg "Graphviz sequence unit attributes"
                   "deposit-edge-color"))
      (fset:with
       (concatenate 'string "units" "edge-color-by" "interface")
       (get-option cfg "Graphviz sequence unit attributes"
                   "interface-edge-color"))
      (fset:with
       (concatenate 'string "units" "edge-fontcolor-by" "deposit")
       (get-option cfg "Graphviz sequence unit attributes"
      ...
      (fset:with
       (concatenate 'string "units" "edge-penwidth-by" "deposit")
       (get-option cfg "Graphviz sequence unit attributes"
                   "deposit-edge-penwidth"))
      (fset:with
       (concatenate 'string "units" "edge-penwidth-by" "interface")
       (get-option cfg "Graphviz sequence unit attributes"
                   "interface-edge-penwidth"))
      (fset:with
       (concatenate 'string "units" "edge-style-by" "deposit")
       (get-option cfg "Graphviz sequence unit attributes"
                   "deposit-edge-style"))
      (fset:with
       (concatenate 'string "units" "edge-color-by" "interface")
      ...
      (fset:with
       (concatenate 'string "adjacent" "edge-penwidth-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-edge-penwidth"))
      (fset:with
       (concatenate 'string "adjacent" "edge-penwidth-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-edge-penwidth"))
      (fset:with
       (concatenate 'string "adjacent" "edge-penwidth-by" "reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "reachable-edge-penwidth"))
      (fset:with
       (concatenate 'string "adjacent" "edge-penwidth-by" "not-adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-edge-penwidth"))
      (fset:with
       (concatenate 'string "adjacent" "edge-color-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-edge-color"))
      (fset:with
       (concatenate 'string "adjacent" "edge-color-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-edge-color"))
      (fset:with
       (concatenate 'string "adjacent" "edge-color-by" "reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "reachable-edge-color"))
      (fset:with
       (concatenate 'string "adjacent" "edge-color-by" "not-adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-edge-color"))
      (fset:with
       (concatenate 'string "adjacent" "edge-style-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-edge-style"))
      (fset:with
       (concatenate 'string "adjacent" "edge-style-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-edge-style"))
      (fset:with
       (concatenate 'string "adjacent" "edge-style-by" "reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "reachable-edge-style"))
      (fset:with
       (concatenate 'string "adjacent" "edge-style-by" "not-adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-edge-style"))
      (fset:with
       (concatenate 'string "adjacent" "edge-fontcolor-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-edge-fontcolor"))
      (fset:with
       (concatenate 'string "adjacent" "edge-fontcolor-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-edge-fontcolor"))
      (fset:with
       (concatenate 'string "adjacent" "edge-fontcolor-by" "reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "reachable-edge-fontcolor"))
      (fset:with
       (concatenate 'string "adjacent" "edge-fontcolor-by" "not-adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-edge-fontcolor"))
      (fset:with "penwidth-max" (get-option cfg "Graphviz sequence edge attributes"
                                            "penwidth-max"))
      (fset:with "penwidth-min" (get-option cfg "Graphviz sequence edge attributes"
                                            "penwidth-min"))
      (fset:with "penwidth" (get-option cfg "Graphviz sequence edge attributes"
                                        "penwidth"))
      (fset:with "arrowhead" (get-option cfg "Graphviz sequence edge attributes"
                                         "arrowhead"))
      (fset:with "fontcolor" (get-option cfg "Graphviz sequence edge attributes"
                                         "fontcolor"))
      (fset:with "fontsize" (get-option cfg "Graphviz sequence edge attributes"
                                        "fontsize"))
      (fset:with "fontname" (get-option cfg "Graphviz sequence edge attributes"
                                        "fontname"))
      (fset:with "color" (get-option cfg "Graphviz sequence edge attributes"
                                     "color"))
      (fset:with "style" (get-option cfg "Graphviz sequence edge attributes"
                                     "style"))
      (fset:with "colorscheme" (get-option cfg "Graphviz sequence edge attributes"
                                            "colorscheme"))))
...
value, and values taken from the corresponding user configuration option."
  (-> (fset:empty-map 0)
      (fset:with
       (concatenate 'string "units" "node-fill-by" "deposit")
       (get-option cfg "Graphviz sequence unit attributes"
                   "deposit-node-fill"))
      (fset:with
       (concatenate 'string "units" "node-fill-by" "interface")
       (get-option cfg "Graphviz sequence unit attributes"
                   "interface-node-fill"))
      (fset:with
       (concatenate 'string "units" "node-shape-by" "deposit")
       (get-option cfg "Graphviz sequence unit attributes"
                   "deposit-node-shape"))
      (fset:with
       (concatenate 'string "units" "node-shape-by" "interface")
       (get-option cfg "Graphviz sequence unit attributes"
                   "interface-node-shape"))
      (fset:with
       (concatenate 'string "units" "node-color-by" "deposit")
       (get-option cfg "Graphviz sequence unit attributes"
                   "deposit-node-color"))
      (fset:with
       (concatenate 'string "units" "node-color-by" "interface")
       (get-option cfg "Graphviz sequence unit attributes"
                   "interface-node-color"))
      (fset:with
       (concatenate 'string "units" "node-penwidth-by" "deposit")
       (get-option cfg "Graphviz sequence unit attributes"
                   "deposit-node-penwidth"))
      (fset:with
       (concatenate 'string "units" "node-penwidth-by" "interface")
       (get-option cfg "Graphviz sequence unit attributes"
                   "interface-node-penwidth"))
      (fset:with
       (concatenate 'string "units" "node-style-by" "deposit")
       (get-option cfg "Graphviz sequence unit attributes"
                   "deposit-node-style"))
      (fset:with
       (concatenate 'string "units" "node-style-by" "interface")
       (get-option cfg "Graphviz sequence unit attributes"
                   "interface-node-style"))
      (fset:with
       (concatenate 'string "adjacent" "node-fill-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-node-fill"))
      (fset:with
       (concatenate 'string "adjacent" "node-fill-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-node-fill"))
      (fset:with
       (concatenate 'string "adjacent" "node-fill-by" "not-adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-node-fill"))
      (fset:with
       (concatenate 'string "adjacent" "node-shape-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-node-shape"))
      (fset:with
       (concatenate 'string "adjacent" "node-shape-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-node-shape"))
      (fset:with
       (concatenate 'string "adjacent" "node-shape-by" "not-adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-node-shape"))
      (fset:with
       (concatenate 'string "adjacent" "node-color-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-node-color"))
      (fset:with
       (concatenate 'string "adjacent" "node-color-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-node-color"))
      (fset:with
       (concatenate 'string "adjacent" "node-color-by" "not-adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-node-color"))
      (fset:with
       (concatenate 'string "adjacent" "node-penwidth-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-node-penwidth"))
      (fset:with
       (concatenate 'string "adjacent" "node-penwidth-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-node-penwidth"))
      (fset:with
       (concatenate 'string "adjacent" "node-penwidth-by" "not-adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-node-fill"))
      (fset:with
       (concatenate 'string "adjacent" "node-style-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-node-style"))
      (fset:with
       (concatenate 'string "adjacent" "node-style-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-node-style"))
      (fset:with
       (concatenate 'string "adjacent" "node-style-by" "not-adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-node-style"))
      (fset:with
       (concatenate 'string "reachable" "node-fill-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-node-fill"))
      (fset:with
       (concatenate 'string "reachable" "node-fill-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-node-fill"))
      (fset:with
       (concatenate 'string "reachable" "node-fill-by" "reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "reachable-node-fill"))
      (fset:with
       (concatenate 'string "reachable" "node-fill-by" "not-reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-node-fill"))
      (fset:with
       (concatenate 'string "reachable" "node-shape-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-node-shape"))
      (fset:with
       (concatenate 'string "reachable" "node-shape-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-node-shape"))
      (fset:with
       (concatenate 'string "reachable" "node-shape-by" "reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "reachable-node-shape"))
      (fset:with
       (concatenate 'string "reachable" "node-shape-by" "not-reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-node-shape"))
      (fset:with
       (concatenate 'string "reachable" "node-color-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-node-color"))
      (fset:with
       (concatenate 'string "reachable" "node-color-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-node-color"))
      (fset:with
       (concatenate 'string "reachable" "node-color-by" "reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "reachable-node-color"))
      (fset:with
       (concatenate 'string "reachable" "node-color-by" "not-reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-node-color"))
      (fset:with
       (concatenate 'string "reachable" "node-penwidth-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-node-penwidth"))
      (fset:with
       (concatenate 'string "reachable" "node-penwidth-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-node-penwidth"))
      (fset:with
       (concatenate 'string "reachable" "node-penwidth-by" "reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "reachable-node-penwidth"))
      (fset:with
       (concatenate 'string "reachable" "node-penwidth-by" "not-reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-node-penwidth"))
      (fset:with
       (concatenate 'string "reachable" "node-style-by" "origin")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "origin-node-style"))
      (fset:with
       (concatenate 'string "reachable" "node-style-by" "adjacent")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "adjacent-node-style"))
      (fset:with
       (concatenate 'string "reachable" "node-style-by" "reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "reachable-node-style"))
      (fset:with
       (concatenate 'string "reachable" "node-style-by" "not-reachable")
       (get-option cfg "Graphviz sequence reachability attributes"
                   "not-reachable-node-style"))
      (fset:with "fillcolor" (get-option cfg "Graphviz sequence node attributes"
                                         "fillcolor"))
      (fset:with "penwidth-max" (get-option cfg "Graphviz sequence node attributes"
                                            "penwidth-max"))
      (fset:with "penwidth-min" (get-option cfg "Graphviz sequence node attributes"
                                            "penwidth-min"))
      (fset:with "penwidth" (get-option cfg "Graphviz sequence node attributes"
                                        "penwidth"))
      (fset:with "fontname" (get-option cfg "Graphviz sequence node attributes"
                                        "fontname"))
      (fset:with "fontcolor" (get-option cfg "Graphviz sequence node attributes"
                                         "fontcolor"))
      (fset:with "fontsize" (get-option cfg "Graphviz sequence node attributes"
                                        "fontsize"))
      (fset:with "color" (get-option cfg "Graphviz sequence node attributes"
                                     "color"))
      (fset:with "style" (get-option cfg "Graphviz sequence node attributes"
                                     "style"))
      (fset:with "shape" (get-option cfg "Graphviz sequence node attributes"
                                            "shape"))))

(defun lookup-option (map value &optional option class)
  (cond
    ((and option class) (fset:@ map (concatenate 'string value option class)))
    (option (fset:@ map (concatenate 'string value option)))
    (t (fset:@ map value))))
...
(defun graphviz-node-style (index)
  ...
  (let ((map
         (-> (fset:empty-map 0)        (fset:with 0 "solid")
             (fset:with 1 "dashed")    (fset:with 2 "dotted")
             (fset:with 3 "bold")      (fset:with 4 "rounded")
             (fset:with 5 "diagonals") (fset:with 6 "filled")
             (fset:with 7 "filled")    (fset:with 8 "striped")
             (fset:with 9 "wedged"))))
    (fset:lookup map (mod index (fset:size map)))))
...
shape."
  (let ((map
         (-> (fset:empty-map 0)              (fset:with 0  "box")
             (fset:with 1  "polygon")        (fset:with 2  "ellipse")
             (fset:with 3  "egg")            (fset:with 4  "triangle")
             (fset:with 5  "diamond")        (fset:with 6  "oval")
             (fset:with 7  "circle")         (fset:with 8  "point")
             (fset:with 9  "trapezium")      (fset:with 10  "parallelogram")
             (fset:with 11  "house")         (fset:with 12  "pentagon")
             (fset:with 13  "hexagon")       (fset:with 14  "septagon")
             (fset:with 15  "octagon")       (fset:with 16  "doublecircle")
             (fset:with 17  "doubleoctagon") (fset:with 18  "tripleoctagon")
             (fset:with 19  "invtriangle")   (fset:with 20  "invtrapezium")
             (fset:with 21  "invhouse")      (fset:with 22  "Mdiamond")
             (fset:with 23  "Msquare")       (fset:with 24  "Mcircle")
             (fset:with 25  "square")        (fset:with 26  "star")
             (fset:with 27  "underline")     (fset:with 28  "note")
             (fset:with 29  "tab")           (fset:with 30  "folder")
             (fset:with 31  "box3d")         (fset:with 32  "component")
             (fset:with 33  "cds")           (fset:with 34  "signature")
             (fset:with 35  "rpromoter")     (fset:with 36  "rarrow")
             (fset:with 37  "larrow")        (fset:with 38  "lpromoter"))))
...
(defun graphviz-color-string (index scheme &optional range)
  ...
  (let ((local-scheme scheme))
    (etypecase index
      (integer
       (unless (brewer-colorscheme-distinctions scheme :member t)
         (error "Error: ~s does not indicate a Brewer colorscheme.~&" scheme))
       (unless range (error "Error: No range for `graphviz-color-string'.~&"))
       (let ((brewer-range (if (> range (brewer-colorscheme-distinctions scheme))
                               (brewer-colorscheme-distinctions scheme) range)))

         (format nil "/~a~s/~s" scheme brewer-range (1+ (mod index brewer-range)))))
      (string
       (unless (fset:contains? (fset:set "x11" "svg" "solarized") scheme)
...
(defun graphviz-hsv-string (color)
  ...
  (let ((hsv-color (as-hsv color)))
    (format nil "~,3f ~,3f ~,3f" (/ (hsv-hue hsv-color) 360)
            (hsv-saturation hsv-color) (hsv-value hsv-color))))
...
(defun graphviz-color-from-ramp (index color1 color2 steps)
  ...
  color string."
  (when (> index steps) (error "Error: Index out of bounds."))
  (let ((c1 (eval (symbolicate #\+ (string-upcase color1) #\+)))
        (c2 (eval (symbolicate #\+ (string-upcase color2) #\+)))
        (alpha (/ index steps)))
...
the possibly modified GRAPH."
  (let ((ret (graph:copy graph)))
    ...
    (dolist (node
              (read-table (get-option cfg "Input files" "contexts")
...
the possibly modified GRAPH."
  (let ((ret (graph:copy graph)))
    ...
    (dolist (arc (read-table
                  (get-option cfg "Input files" "observations")
  ...
  (let ((ret (graph:copy graph)))
    (when
      ...
      (dolist (part (read-table (get-option cfg "Input files" "inferences")
                                (get-option cfg "Input file headers"
                                            "inferences")
                                verbose))
        (graph:merge-nodes
         ret (symbolicate (nth 1 part)) (symbolicate (nth 0 part))
...
(defun graphviz-make-ranks (cfg &optional (verbose t))
  (let ((ranks))
    (when verbose (format t "Setting ranks.~&"))
    (when
        (get-option cfg "General configuration" "assume-correlations"
                    :type :boolean)
      (appendf
...
and the value is either the period or phase of the context."
  (let ((ht (fset:empty-map))
    ...
    (mapcar
     ...
     other-table)
    (mapcar
     #'(lambda (x) (setf ret (fset:with ret (symbolicate (nth 0 x))
...
(defun alist-to-map (alist)
  ...
  (let ((ret (fset:empty-map)))
    (mapc #'(lambda (pair) (setq ret (fset:with ret (car pair) (cdr pair)))) alist)
    ret))
...
(defstruct (archaeological-sequence
             (:print-function print-archaeological-sequence))
      ...
      (let ((class (get-option cfg "Graphviz sequence classification"
        ...
        (unless (emptyp class)
          (setf (archaeological-sequence-classifiers ret)
                (fset:with (archaeological-sequence-classifiers ret)
                           class (make-classifier class cfg ret verbose))))))
    ret))
...
`reachable', `adjacent', `periods', `phases', `units', or `levels'."
  (cond
    ((string= classifier-type "units")
     (when verbose (format t "Creating units classification.~&"))
     (context-type-to-map
      (read-table
       (get-option cfg "Input files" "contexts")
       (get-option cfg "Input file headers" "contexts") verbose)))

    ((string= classifier-type "distance")
     (when verbose (format t "Creating distance classification.~&"))
     (let* ((m (create-distance-matrix
               cfg (archaeological-sequence-graph seq)))
            (i (make-node-index (archaeological-sequence-graph seq)))
            (o (symbolicate
                (get-option cfg "General configuration" "reachable-from")))
            (new-key (fset:@ i o))
            (map (fset:empty-map 0)))
       (fset:do-map (key val i)
         (let ((new-val (round
                         (min
                          (graph-matrix:matrix-ref m new-key val)
                          (graph-matrix:matrix-ref m val new-key)))))
           (unless (graph-matrix:infinitep new-val m)
             (setf map (fset:with map key new-val)))))
       (when verbose (format t "Created distance classification.~&"))
       map))

    ((string= classifier-type "reachable")
     (when verbose (format t "Creating reachability classification.~&"))
     (let ((m (create-reachability-matrix
               cfg (archaeological-sequence-graph seq)))
           (i (make-node-index (archaeological-sequence-graph seq)))
           (o (get-option cfg "General configuration" "reachable-from"))
           (map (fset:empty-map)))
       (fset:do-map (key val i)
         (setf map
    ...
    ((string= classifier-type "adjacent")
     (when verbose (format t "Creating adjacency classification.~&"))
     (let ((m (create-adjacency-matrix
               cfg (archaeological-sequence-graph seq)))
           (i (make-node-index (archaeological-sequence-graph seq)))
           (o (get-option cfg "General configuration" "reachable-from"))
           (map (fset:empty-map)))
    ...
    ((member classifier-type '("periods" "phases") :test 'string=)
     (when verbose
       (format t "Creating classification by ~a.~&" classifier-type))
     (tables-to-map
      (read-table (get-option cfg "Input files" "contexts")
                  (get-option cfg "Input file headers" "contexts")
                  verbose)
    ...
    ((string= classifier-type "levels")
     (alist-to-map
      (graph:levels (archaeological-sequence-graph seq) :alist 't)))
    (t (error "The classifier '~a' is not known." classifier-type))))

(defun create-chronology-graph (distance-matrix cfg seq &optional (verbose t))
  "Create and return a chronology graph, given a DISTANCE-MATRIX and a
configuration CFG.  If VERBOSE, then advertise progress."
  ...
  (let ((ret (make-instance 'graph:digraph))
        (event-table (read-table (get-option cfg "Input files" "events")
                                 ...
                                 verbose))
        (event-order-table
         (when (not (emptyp (get-option cfg "Input files" "event-order")))
    ...
    (let ((i (make-node-index (archaeological-sequence-graph seq)))
      ...
      (dolist (row event-table)
        (push (symbolicate (nth 1 row)) events)
        (graph:add-edge ret
...
COLOR-SCALE is not a map key and non-nil otherwise."
  (let ((map (-> (fset:empty-map)
                 (fset:with "accent" 8)  (fset:with "blues" 9)
                 (fset:with "brbg" 11)
                 (fset:with "bugn" 9)    (fset:with "bupu" 9)
                 (fset:with "dark2" 8)   (fset:with "gnbu" 9)
                 (fset:with "greens" 9)  (fset:with "greys" 9)
                 (fset:with "oranges" 9) (fset:with "orrd" 9)
                 (fset:with "paired" 12) (fset:with "pastel1" 9)
                 (fset:with "pastel2" 8) (fset:with "piyg" 11)
                 (fset:with "prgn" 11)   (fset:with "pubu" 9)
                 (fset:with "pubugn" 9)  (fset:with "puor" 11)
                 (fset:with "purd" 9)    (fset:with "purples" 9)
                 (fset:with "rdbu" 11)   (fset:with "rdgy" 11)
                 (fset:with "rdpu" 9)    (fset:with "rdylbu" 11)
                 (fset:with "rdylgn" 11) (fset:with "reds" 9)
                 (fset:with "set1" 9)    (fset:with "set2" 8)
                 (fset:with "set3" 12)   (fset:with "spectral" 11)
                 (fset:with "ylgn" 9)    (fset:with "ylgnbu" 9)
                 (fset:with "ylorbr" 9)  (fset:with "ylorrd" 9))))
    (if member
...
specification is prefixed with an octothorp."
  (assert (eq (length hex) 6) (hex) "Error: ~a is not 6 characters." hex)
  (format nil "#~a" hex))
  ...
  (if (get-option cfg "General configuration" "add-missing-interfaces"
      ...
      (let ((hiatus)
        ...
        (dolist (context contexts)
          (setf context-lookup
                (fset:with context-lookup (symbolicate (nth 0 context))
                           (nth 1 context))))
        (dolist (edge (graph:edges g))
          (and (string= (fset:@ context-lookup (nth 0 edge)) "deposit")
               ...
               (push edge hiatus)))
        (when verbose
          (format t "Adding ~:d interfaces.~&" (list-length hiatus)))
        (dolist (edge hiatus)
...
(defun write-sequence-graph-to-dot-file (seq &optional (verbose t))
  (graph-dot:to-dot-file
   ...
   (get-option (archaeological-sequence-configuration seq)
               "Output files" "sequence-dot")
   :ranks (graphviz-make-ranks (archaeological-sequence-configuration seq))
   :attributes
   (list
    (cons :style
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "style") ""))
    (cons :dpi
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "dpi") "96.0"))
    (cons :URL
          (<-dot-graph
           (if (get-option (archaeological-sequence-configuration seq)
                           "General configuration" "url-include"
                           :type :boolean)
               (get-option (archaeological-sequence-configuration seq)
                           "General configuration" "url-default") "") ""))
    (cons :margin
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "margin") ""))
    (cons :bgcolor
          (<-dot-graph (user-color seq "Graphviz sequence graph attributes"
                                   "bgcolor") ""))
    (cons :fontname
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "fontname") ""))
    (cons :fontsize
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "fontsize") "14.0"))
    (cons :fontcolor
          (<-dot-graph (user-color seq "Graphviz sequence graph attributes"
                                   "fontcolor") "black"))
    (cons :splines
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "splines") ""))
    (cons :page
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "page") ""))
    (cons :size
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "size") ""))
    (cons :ratio
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "ratio") ""))
    (cons :label
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "label") ""))
    (cons :labelloc
          (<-dot-graph
           (get-option (archaeological-sequence-configuration seq)
                       "Graphviz sequence graph attributes" "labelloc") "b")))
   :edge-attrs
   (list
    (cons :style
          (<-dot-edge seq "edge-style-by" "style" "solid"))
    (cons :arrowhead
          (<-dot-edge seq nil "arrowhead" ""))
    (cons :color
          (<-dot-edge seq "edge-color-by" "color" "black"))
    (cons :fontname
          (<-dot-edge seq nil "fontname" "Times-Roman"))
    (cons :fontsize
          (<-dot-edge seq nil "fontsize" "14.0"))
    (cons :fontcolor
          (<-dot-edge seq "edge-fontcolor-by" "fontcolor" "black"))
    (cons :penwidth
          (<-dot-edge seq "edge-penwidth-by" "penwidth" ""))
    ;; (cons :URL (lambda (x) (format-attribute
    ;;                    (if *url-include* (url-decode (fset:@ arc-urls x)) "")
    ;;                    :preserve-case t :quote t))))
      ) ;; close edge-attrs list
   :node-attrs
   (list
    (cons :shape
          (<-dot-node seq "node-shape-by" "shape" "box"))
    (cons :style
          (<-dot-node seq "node-style-by" "style" "filled"))
    (cons :fontname
          (<-dot-node seq nil "fontname" "Times-Roman"))
    (cons :fontsize
          (<-dot-node seq nil "fontsize" "14.0"))
    (cons :color
          (<-dot-node seq "node-color-by" "color" "black"))
    (cons :fillcolor
          (<-dot-node seq "node-fill-by" "fillcolor" "white"))
    (cons :fontcolor
          (<-dot-node seq nil "fontcolor" "black" ))
    (cons :penwidth
          (<-dot-node seq "node-penwidth-by" "penwidth" "1.0"))
    ;; (cons :URL (lambda (x) (format-attribute
    ;;                    (if *url-include*
